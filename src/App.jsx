...
function App() {
  ...
  // Weight hooks 
  const [fromWeightUnit, setFromWeightUnit] = useState('kilos');
  const [toWeightUnit, setToWeightUnit] = useState('pounds');
  const [inputWeightValue, setInputWeightValue] = useState('');
  const [weightResult, setWeightResult] = useState('');
  // Handlers to set result 
  function handleConvertLength() {
    setLengthResult(ConvertLen(inputLengthValue, fromLengthUnit, toLengthUnit));
  }
  function handleConvertTemperature() {
    setResult(ConvertTemp(inputValue, fromUnit, toUnit));
  }
  function handleConvertWeight() {
    setWeightResult(ConvertWeight(inputWeightValue, fromWeightUnit, toWeightUnit));
  }
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 flex items-center justify-center">
      <div className="space-y-16 w-full max-w-md bg-slate-800/80 border border-slate-700 rounded-xl p-6 shadow-lg">
        ...
        <div className="flex justify-between">
          ...
          <ChoiceButton onChoice={() => setCurrMenu('weight')} classItem={"cursor-pointer text-white bg-gradient-to-r from-purple-500 to-pink-500 hover:bg-gradient-to-l focus:ring-4 focus:outline-none focus:ring-purple-200 dark:focus:ring-purple-800 font-medium rounded-lg text-sm px-4 py-2.5 text-center leading-5"}>Weight</ChoiceButton>
        </div>
          ...
          <div>
            ...
            <div>
              <ConverterButton
                ...
                className={"w-full mt-4 cursor-pointer text-white bg-gradient-to-br from-green-400 to-blue-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-200 dark:focus:ring-green-800 font-medium rounded-lg text-sm px-4 py-2.5 text-center leading-5"}>
                Convert
              </ConverterButton>
            </div>
            {/*------Result------- */}
            <div className="mt-4 p-3 rounded-lg bg-slate-700 border border-slate-600">
              <span className="font-semibold text-white">
                {lengthResult === '' ? 'Result will show here' : `${lengthResult} ${toLengthUnit}`}
              </span>
            </div>
          </div>
        )}
        ...
        {currMenu === 'temperature' && (
          <div>
            ...
            <div className="mt-4 p-3 rounded-lg bg-slate-700 border border-slate-600">
              <span className="font-semibold text-white">
                {result === '' ? 'Result will show here' : `${result} ${toUnit}`}
              </span>
            </div>
          </div>
        )}
        ...
        {currMenu === 'weight' && (
          <div>
            ...
            <div className="flex">
              <div className="flex items-center gap-4">
                <label htmlFor="weight-unit1">From</label>
                <select
                  id="weight-unit1"
              ...
              <div className="flex items-center gap-8 ml-8">
                <label htmlFor="weight-unit2">To</label>
                <select
                  id="weight-unit2"
                  value={toWeightUnit}
            ...
            <div>
              <ConverterButton
                ...
                className={"w-full mt-4 cursor-pointer text-white bg-gradient-to-r from-purple-500 to-pink-500 hover:bg-gradient-to-l focus:ring-4 focus:outline-none focus:ring-purple-200 dark:focus:ring-purple-800 font-medium rounded-lg text-sm px-4 py-2.5 text-center leading-5"}
              >
                Convert
              </ConverterButton>
            </div>
            ...
            <div className="mt-4 p-3 rounded-lg bg-slate-700 border border-slate-600">
              <span className="font-semibold text-white">
                {weightResult === '' ? 'Result will show here' : `${weightResult} ${toWeightUnit}`}
              </span>
            </div>
          </div>
        )}
